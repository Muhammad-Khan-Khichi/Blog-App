...
import authService from "../../appwrite/auth"; // ✅ add this
...
export default function PostForm({ post }) {
  ...
  // ✅ helper to ensure a session exists before any Appwrite API call
  const ensureSession = async () => {
    let user = await authService.getCurrentUser();
    if (!user) {
      console.warn("⚠️ No active session found — creating anonymous session...");
  ...
  const submit = async (data) => {
    try {
      // ✅ Ensure session before uploading or creating documents
      const user = await ensureSession();

      if (post) {
        const file = data.image[0]
          ? await appwriteService.uploadFile(data.image[0], user.$id)
          : null;
        ...
        const file = await appwriteService.uploadFile(data.image[0], user.$id);
        if (file) {
      ...
      alert("Upload failed. Please log in or check permissions.");
    }
  };
